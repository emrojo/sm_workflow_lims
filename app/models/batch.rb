...
class Batch < ActiveRecord::Base
  ...
  class Creator
    ...
    attr_reader :comment, :assets, :study, :workflow, :asset_type, :pipeline_destination, :cost_code

    def self.create!(*args)
      self.new(*args).do!
    end

    def initialize(study:,assets:,asset_type:,workflow:,pipeline_destination:,cost_code:,comment:nil)
      @study = study
      @assets = assets
  ...
  class Updater
    ...
    def do!
      ActiveRecord::Base.transaction do
        batch.assets.update_all(study:study,workflow_id:workflow,pipeline_destination_id:pipeline_destination,cost_code:cost_code,comment_id:comment_object)
      end
      batch
    end
